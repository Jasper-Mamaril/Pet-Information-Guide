...
class Get{
    ...
    public function get_specific($table, $condition = null){
        ...
        if($res['code']==200){
            return $this->gm->returnPayload($res['data'], "success", "Succesfully retieved petlist details", $res['code']);
        }

        return $this->gm->returnPayload(null, "failed", "failed to retrieve petlist details", $res['code']);
    }

    // public function get_time($table, $condition = null){
    //     $sql = "SELECT petname FROM $table";
    //     if($condition!=null){
    //         $sql .= " WHERE {$condition}";
    //     }
    ...
    public function get_reminderlist($table, $condition = null){
        $sql = "SELECT reminders_tbl.*, SUBTIME(`time`, '4:00:00') AS timeset FROM `reminders_tbl`";
        if($condition!=null){
            $sql .= " WHERE {$condition}";
        }
